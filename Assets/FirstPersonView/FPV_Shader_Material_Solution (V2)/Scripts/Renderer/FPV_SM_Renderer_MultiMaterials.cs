...
{
    [AddComponentMenu("FPV/Shader-Material Setup/FPV Renderer Multi-Material")]
    public class FPV_SM_Renderer_MultiMaterials: FPV_Renderer
    {
        /// <summary>
        ...
        public FPV_SM_Material[] fpvMaterials;
        ...
        private void PrepareFPVMaterials()
        {
            if(fpvMaterials == null || fpvMaterials.Length == 0)
            {
                ...
                fpvMaterials = new FPV_SM_Material[_render.sharedMaterials.Length];
                for (int i = 0; i < fpvMaterials.Length; i++)
                {
                    fpvMaterials[i] = new FPV_SM_Material();
                }
            }
            else if (fpvMaterials.Length != _render.sharedMaterials.Length)
            {
                ...
                FPV_SM_Material[] newFpvMaterials = new FPV_SM_Material[_render.sharedMaterials.Length];
                ...
                for (int i = 0; i < fpvMaterials.Length; i++)
                {
                    ...
                    {
                        newFpvMaterials[i] = new FPV_SM_Material();
                    }
                }
        ...
        private void InitialteFPVMaterials()
        {
            ...
            for (int i = 0; i < fpvMaterials.Length; i++)
            {
                FPV_SM_Material mat = fpvMaterials[i];
                ...
                mat.FirstPersonViewMaterial.EnableKeyword("FIRSTPERSONVIEW");
            }
